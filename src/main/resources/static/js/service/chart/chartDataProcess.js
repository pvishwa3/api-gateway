...
var chartDataProcess = function(chartConfig,casted_keys, casted_values, aggregate_data,newValuesConfig) {
	...
	cols = chartConfig.values[0].cols;
	...
	for (var i = 0; i < casted_values.length; i++) {
		...
		for (var j = 0; j < casted_keys.length; j++) {
			if (!_.isUndefined(aggregate_data[i][j])) {
				...
				if(cols[i] && cols[i].configuration && cols[i].configuration.format==="bytes"){
					table_data[j][i + keyLength] = {
							property: 'data',
							data: formatBytes(parseInt(raw)),
							raw: raw
					};
				}else if(cols[i] && cols[i].configuration && cols[i].configuration.format==='date'){
					table_data[j][i + keyLength] = {
							property: 'data',
							data: moment(parseInt(raw)).format('LLLL')
					};
				}else{
					table_data[j][i + keyLength] = {
							property: 'data',
