 ...
 */
'use strict';
app.service('dataService', function ($http, $q, updateService) {
	...
	this.getDataSeries = function (datasource, query, datasetId, chartConfig,widgetId, callback, reload,option) {
		chartConfig = angular.copy(chartConfig);
		var dataSeries;
		var cfg = {rows: [], columns: [], filters: []};
		if(chartConfig.config){
			updateService.updateConfig(chartConfig.config);
			dataSeries =  getDataSeries(chartConfig.config);
			
			
			cfg.rows = getDimensionConfig(chartConfig.config.keys);
			cfg.columns = getDimensionConfig(chartConfig.config.groups);
			cfg.filters = getDimensionConfig(chartConfig.config.filters);
			cfg.filters = cfg.filters.concat(getDimensionConfig(chartConfig.config.boardFilters));
			...
			dataSeries =  getDataSeries(chartConfig);
			cfg.rows = getDimensionConfig(chartConfig.keys);
			cfg.columns = getDimensionConfig(chartConfig.groups);
			cfg.filters = getDimensionConfig(chartConfig.filters);
		...
		$http.post("/siem-core/widget/perview-for-chart", {
			datasourceId: datasource,
			query: angular.toJson(query),
			datasetId: datasetId,
			cfg: angular.toJson(cfg),
			config:angular.toJson(chartConfig),
			reload: reload,
			widgetId : widgetId,
			...
			if(chartConfig.config){
			    ...
			    if(chartConfig.config.chart_type==='table'){

			        var object = new Object();
			        object['data'] = data.data;
			        object['config'] = data.data;
			        result = object;
			    }else{
			         result = castRawData2Series(data.data, chartConfig.config);
			    }


			}else{
				result = castRawData2Series(data.data, chartConfig);
			}
			
			result['columnList'] = data.data.columnList;
			if(chartConfig.config){
				result.chartConfig = chartConfig.config;
			}else{
	...
	var castRawData2Series = function (aggData, chartConfig) {
		...
		var joinedGroups = {};
		var newData = {};
		var chartType = chartConfig.chart_type;
