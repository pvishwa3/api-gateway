app.controller("logsimulationController",['$scope', '$rootScope','logsimulationFactory','corrleationFactory', '$timeout','$ngConfirm','DTOptionsBuilder','DTColumnBuilder','DTColumnDefBuilder','$filter','$location', function($scope, $rootScope, logsimulationFactory,corrleationFactory, $timeout,$ngConfirm,DTOptionsBuilder,DTColumnBuilder,DTColumnDefBuilder,$filter,$location) {
	var self = this;
	...
	self.newsim = { "id":0, "eventDetailsList":[], "ruleName":"","randomEvents": false, "simulationName":"", "simulationDescription":"", "simulationType":"", "eventDetailsId":0, "attackDetailsId":0, "count":1, "correctness":100, "status":"", "scheduleDetails":{ "schedulerFlag":"FALSE", "cronExpression": "" }, "metaFields":[{"key": "log_device", "value": "default"},{"key": "log_type", "value": "default"},{"key": "index_name", "value": "technominds"},{"key": "company", "value": "tmcl"}], "multiLineDetails":{ "multiLineFlag":"FALSE", "multiLinePattern":{"negate":"TRUE", "match":"AFTER", "pattern":""} }, "simulationIP":{"type":"RANDOM IP","cidrValue":"","countryValue":"","range":{"fromIp":"","toIp":""}}, "simulationDate":{ "type":"REAL TIME","range": { "startDate" :"","endDate":""}, "relative": {"timeState":"", "timeFrame":0, "timeMetric":"", "eventCount":1}}};
	self.newEventType = {"eventTypeName":"", "eventTypeId":0};
	self.newLogType = {"logTypeName":"", "logTypeId":0};
	self.newAttackDetails = {"attackId":0,"attackName":"", "eventDetails":[]}
	...
	self.resetEventDetails = function(){
		self.newEventDetails = {"eventName":"", "eventType":"", "logType":"", "messageType":"", "osType":"", "formatType":"PLAIN", "template":"", "eventId":0};
	}
//	$scope.vm = {};
//	$scope.vm.dataInstance = {};  
//	$scope.vm.dataOptions = DTOptionsBuilder.newOptions().withPaginationType('full_numbers').withDisplayLength(25).withOption('order', [1, 'asc'])
//	.withOption('lengthMenu', [25,50, 100, 150, 200])
//	.withOption('scrollY', $( window ).height()-350)
//	.withOption('scrollCollapse', true);;
//	$scope.vm.dataColumnDefs = [
//		DTColumnDefBuilder.newColumnDef(0).notSortable(),
//		DTColumnDefBuilder.newColumnDef(6).withOption('searchable', false)
//		];
	
	self.markedData = [];
	self.deleteAllBySelSimulationById=function(ids){
		 $ngConfirm({ 
			 	animation: 'top',
	            ...
	            buttons: {
	                delete: {
	                    ...
	                    action: function(scope, button){
							logsimulationFactory.deleteAllSimulationById(self.markedData).then(function(response){
								for (var i = self.markedData.length - 1; i >= 0; i--) {
									self.markedData.splice(self.allSchedules[i], 1);
						        }
								//console.log(response.data);
								self.getScheduledSimulations();
								self.alertMessages.push({ type: 'alert-success', msg: 'Scheduled Events Deleted' });
								 $timeout(function () {
						      			self.alertMessages.splice(0, 1);
						      		}, 2000);

							},function(err){
								self.alertMessages.push({ type: 'alert-warning', msg: 'Unable to delete Scheduled Events' });
								 $timeout(function () {
						      			self.alertMessages.splice(0, 1);
						      		}, 2000);
							});
	...
	self.enableDisableSimulation = function(status){
		logsimulationFactory.enableDisableSimualtion(status,self.markedData).then(function(response){
			self.alertMessages.push({ type: 'alert-success', msg: 'Enabled Or Disabled Simulations' });
	    	   $timeout(function () {
				self.alertMessages.splice(0, 1);
			}, 2000);
			self.getScheduledSimulations();
	...
	self.onDemandSimulationForSel = function(){
		var simulationDetails = [];
		for(var i=0;i<self.markedData.length;i++){
			for(var j=0;j<self.allSchedules.length;j++){
				if(self.allSchedules[j].id == self.markedData[i]){					
					simulationDetails.push(self.allSchedules[j]);
				}
			}
		}
		
		logsimulationFactory.onDemandSimulationForSelection(simulationDetails).then(function(response){
			self.alertMessages.push({ type: 'alert-success', msg: 'Simulation done successfully' });
	    	   $timeout(function () {
				self.alertMessages.splice(0, 1);
			}, 2000);
			self.getScheduledSimulations();
	...
	self.markedData = [];
	self.checkUncheckHeader = function (id,flag) {
		if(self.markedData.length  == 0){
			self.markedData.push(id);
		}else{
			if(self.markedData.indexOf(id) == -1 && flag === true){
				self.markedData.push(id);				
			}else{
				self.markedData.splice(self.markedData.indexOf(id),1);
			}
		}
		console.log(self.markedData);
    };
    self.markedData1=[];
	  self.checkUncheckAll = function (flag) {
	    	self.markedData = [];
	    	self.markedData1=[];
	       if(flag == true){
	    	   for(var i=0;i<self.allSchedules.length;i++){
	    		   self.allSchedules[i].selected = true;
	    		   self.markedData.push(self.allSchedules[i].id);
	    		   //self.markedData1.push(self.allSchedules[i]);
	    		  
	    	   }
	    	   //console.log(self.markedData1);
	    	   
	       }else{
	    	   for(var i=0;i<self.allSchedules.length;i++){
	    		   self.allSchedules[i].selected = false;
	      	   }
	    	   self.markedData = [];
	       }
	       console.log(self.markedData);
	...
	$scope.obj = {data: self.newEventDetails.template, options: {mode: 'tree'}};
...
//	self.getScheduledSimulations();
	 ...
	 /*self.ondemandGeneration = function(){
...
//		self.newEventDetails.delete("logType");
	...
	self.saveNewAttackDetails = function() {
		...
		if (self.newAttackDetails.attackName == '' || self.newAttackDetails.attackName == undefined ){
			 self.alertMessages.push({ type: 'alert-danger', msg: 'All fields are manditory' });
	    	   $timeout(function () {
				self.alertMessages.splice(0, 1);
			}, 2000);
		}else if(self.newAttackDetails.eventDetails.length == 0){
				self.alertMessages.push({ type: 'alert-danger', msg: 'Please select atleast one Event' });
		    	   $timeout(function () {
					self.alertMessages.splice(0, 1);
				}, 2000);
		    	   return false;
			...
			var flag = false;
			self.newAttackDetails.eventDetails.forEach(function(item){
				if(item.timeMetric == '' || item.timeMetric == undefined || item.eventCount == '' ||  item.eventCount == undefined || item.selectEvent == '' || item.selectEvent == undefined){
					self.alertMessages.push({ type: 'alert-danger', msg: 'Please enter all the fields in event details' });
			    	   $timeout(function () {
						self.alertMessages.splice(0, 1);
					}, 2000);
			    	flag = true;
...
//	 $scope.eventOptions = DTOptionsBuilder.newOptions().withOption('order', [1, 'asc']).withOption('responsive', true)
...
//		]);
//
//		$scope.atackOptions = DTOptionsBuilder.newOptions().withOption('order', [1, 'asc']).withOption('responsive', true)
//		.withDOM("<'row'<'col-sm-4'l><'col-sm-3 width-200 pull-right'B><'col-sm-5 pull-right'f>>" +"<'row'<'col-sm-12'tr>>" +"<'row'<'col-sm-5'i><'col-sm-7'p>>")
//		.withPaginationType('full_numbers')
//		.withDisplayLength(25)
//		.withOption('scrollY', $( window ).height()-350)
...
//		
//		
//		$scope.fieldOptions = DTOptionsBuilder.newOptions().withOption('order', [1, 'asc']).withOption('responsive', true)
//		.withDOM("<'row'<'col-sm-4'l><'col-sm-3 width-200 pull-right'B><'col-sm-5 pull-right'f>>" +"<'row'<'col-sm-12'tr>>" +"<'row'<'col-sm-5'i><'col-sm-7'p>>")
//		.withPaginationType('full_numbers')
//		.withDisplayLength(25)
//		.withOption('scrollY', $( window ).height()-350)
...
//		]);
//		
//		$scope.simulationOptions = DTOptionsBuilder.newOptions().withOption('order', [1, 'asc']).withOption('responsive', true)
//		.withDOM("<'row'<'col-sm-4'l><'col-sm-3 width-200 pull-right'B><'col-sm-5 pull-right'f>>" +"<'row'<'col-sm-12'tr>>" +"<'row'<'col-sm-5'i><'col-sm-7'p>>")
//		.withPaginationType('full_numbers')
//		.withDisplayLength(25)
//		.withOption('scrollY', $( window ).height()-350)
...
app.filter("eventfilter",function(){
		return function(items,search){
			...
			var object = new Object();
			angular.forEach(items,function(value,key){
				angular.forEach(value.subjects,function(value1,key1){
					if(value1.eventSubType.toLowerCase().includes(search.toLowerCase())){
