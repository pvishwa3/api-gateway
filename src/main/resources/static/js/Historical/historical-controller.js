...
app.controller("historyController", ['$scope', 'widgetService','$rootScope','$timeout','$uibModal','DTOptionsBuilder','DTColumnBuilder','DTColumnDefBuilder','$ngConfirm','$location','$routeParams','Fullscreen','$route','$interval','$window','tagService','colorConfigurationFactory','$filter','alertsFactory','chartService','$compile','investigationPanelFactory','logDevicesFactory','$sessionStorage','settingsFactory',function ($scope, widgetService,$rootScope, $timeout,$uibModal,DTOptionsBuilder, DTColumnBuilder,DTColumnDefBuilder,$ngConfirm,$location,$routeParams,Fullscreen,$route,$interval,$window,tagService,colorConfigurationFactory,$filter,alertsFactory,chartService,$compile,investigationPanelFactory,logDevicesFactory,$sessionStorage,settingsFactory) {
	var self = this;
        ...
        $scope.logFieldsFilter = [];
    		logDevicesFactory.getAllLogFields().then(function(response){
    			...
    			for(var i=0;i<response.data.length;i++){
                				if(response.data[i].fieldName==='events_tags' ||response.data[i].fieldName ==='rules_tags'){
                					data.fields[response.data[i].fieldName+".keyword"] = { type: 'multi',title:response.data[i].fieldName,field:response.data[i].fieldName,choices:self.tagDetails+".keyword"};
                				}else{
                					data.fields[response.data[i].fieldName+".keyword"] = { type: 'term',title:response.data[i].fieldName,field:response.data[i].fieldName+".keyword"};

                				}
                				//{fieldName:response.data.config.filterFields[i]+".keyword"}
                				$scope.logFieldsFilter.push({fieldName:response.data[i].fieldName+".keyword",dataType:response.data[i].dataType});
                			}
...
//	let the grid know which columns and what data to use
    ...
    self.isAgridLoaded = false;


//	lookup the container we want the Grid to use

	self.intializeGrid = function(data){
		...
		if($scope.type === 'Alert'){
			columnDefs.push({headerName:"@timestamp", field: "@timestamp", width: 200,  headerCheckboxSelectionFilteredOnly: true});
			...
			columnDefs.push({headerName:"Assignee",hide:false,field: "assignee", width: 200,enableRowGroup: true,filter: 'agTextColumnFilter',filterParams:{ filterOptions:['contains'],suppressAndOrCondition:true }});
			columnDefs.push({headerName:"Status",hide:false,field: "alert_status", width: 200,enableRowGroup: true,filter: 'agTextColumnFilter',filterParams:{ filterOptions:['contains'],suppressAndOrCondition:true }});
			$scope.headerTitle = "Alerts";

		}else if($scope.type === 'rules'){
			...
			columnDefs.push({colId:0,headerName:"@timestamp", field: "@timestamp", width: 200,});
			columnDefs.push({colId:1,headerName:"event_name", field: "event_name",width: 200,enableRowGroup: true,enablePivot: true});
			columnDefs.push({colId:2,headerName:"event_category", field: "event_category",width: 200,enableRowGroup: true,enablePivot: true});
			columnDefs.push({colId:3,headerName:"log_type", field: "log_type",width: 200,enableRowGroup: true,enablePivot: true});
			columnDefs.push({colId:4,headerName:"log_device", field: "log_device",width: 200,enableRowGroup: true,enablePivot: true});
			$scope.headerTitle = "Events";
		}else if($scope.type === 'case'){
		...
		for(var i=0;i<$scope.logFieldsFilter.length;i++){



			if($scope.logFieldsFilter[i].fieldName=== 'event_time' ||  $scope.logFieldsFilter[i].fieldName === 'event_name' ||  $scope.logFieldsFilter[i].fieldName === 'event_category' || $scope.logFieldsFilter[i].fieldName === 'log_device' || $scope.logFieldsFilter[i].fieldName === 'log_type'){
				//columnDefs.push({headerName:$scope.logFields[i].fieldName, field: $scope.logFields[i].fieldName, width: 150,hide:false});
			}else if(tempArray.indexOf($scope.logFieldsFilter[i].fieldName)==-1){
                    if($scope.logFieldsFilter[i].dataType.toLowerCase() === 'numeric'){
                        columnDefs.push({colId:$scope.logFieldsFilter[i].fieldName,headerName:$scope.logFieldsFilter[i].fieldName.replace(".keyword",""),enableRowGroup: true,enablePivot: true, field: $scope.logFieldsFilter[i].fieldName.replace(".keyword",""), width: 150,hide:true,allowedAggFuncs: ['sum', 'min', 'max','avg'],enableValue: true});
                    }else{
                        columnDefs.push({colId:$scope.logFieldsFilter[i].fieldName,headerName:$scope.logFieldsFilter[i].fieldName.replace(".keyword",""),enableRowGroup: true,enablePivot: true, field: $scope.logFieldsFilter[i].fieldName.replace(".keyword",""), width: 150,hide:true});

                    }
					 tempArray.push($scope.logFieldsFilter[i].fieldName)
			}
...
function calculateTimeRange(startDate,endDate){
	...
	$scope.getHistoricalData = function(){
	 if(!$scope.output){
                 $scope.output = JSON.stringify( {"bool":{"must":[]}});
       }
	    ...
	    $scope.options = {
         ...
         widgetService.getHistoricalData($scope.options).then(function (response) {
              self.intializeGrid(response.data);
         }, function (error) {
	...
	$scope.apply = function(){
        ...
        var formatDate = calculateTimeRange($scope.startDate,$scope.endDate);
		...
		widgetService.getTimeSeriesData($scope.options).then(function (response) {
		...
		var whiteTheme = 'image://data:image/svg+xml;base64,'
            ...
            for(var i=0;i<response.data.data.length;i++){
                seriesData.push(response.data.data[i][0]);
                chartData.push(response.data.data[i][1])
            }
           echarts.registerTheme('shine',localStorage.getItem("themeType") === 'white'? lightTheme:darkTheme);

            var mainChart =  echarts.init(document.getElementById('time-series'), 'shine');

         var format =  moment($scope.startDate).format('LLLL') + " to " + moment($scope.endDate).format('LLLL');

         var option = {
            title: {
                    text: response.data.size+" hits",
                    subtext: format,
                    left: 'center'
                },
                tooltip: {
        ...
        formatter : function(params) {
			let format = calculateTimeRange(parseInt($scope.startDate),parseInt($scope.endDate))
        	return echarts.format.formatTime(format, parseInt(params.name))+'<hr style="margin:0px;padding:0px;" />Count : '+params.data;
        }
    },
              grid:{
                    bottom:"15%",
              ...
              toolbox:{
                      						...
                      						feature : {
                      							dataZoom : {
                      								...
                      								icon:{
                      									back:'a',
                      									zoom:(localStorage.getItem("dashboard-themeType") === 'theme-dark-full') ? darkTheme1 : whiteTheme
                      								}
                      							},
                      							magicType: {
                      								show: true,
                  ...
                  data: seriesData,
                   axisLabel: {
                              ...
                              formatter: function (value, index) {
                              							var format = calculateTimeRange(parseInt($scope.startDate),parseInt($scope.endDate))
              ...
              series: [{
                  ...
                  self.intializeGrid();
                  mainChart.on('datazoom', function (evt) {
                  		var axis = mainChart.getModel().option.xAxis[0];
...
$timeout(function(){
    ...
    $scope.labelName = "Last 24 Hours"
	$scope.apply();
},$sessionStorage.user == undefined?500:0)
...
$scope.getSavedSearches =function(){
     widgetService.getSavedSearches().then(function (response) {
        $scope.savedSearchesData = response.data;
    }, function (error) {
...
$scope.openSearch = function(){
    $scope.historicalSearchModel = {id:0,queryName:'',metaData:'',tableMetaData:''};
    $("#open_search").modal();
}

$scope.openConfiguration = function(data){
    $scope.historicalSearchModel = angular.copy(data);

        var tempObject =  JSON.parse($scope.historicalSearchModel.tableMetaData);
        var tempConfig =  JSON.parse($scope.historicalSearchModel.metaData);
        gridOptions.columnApi.setColumnState(tempObject.colState);
        gridOptions.columnApi.setColumnGroupState(tempObject.groupState);
        gridOptions.api.setSortModel(tempObject.sortState);
        gridOptions.api.setFilterModel(tempObject.filterState);
        gridOptions.columnApi.setPivotMode(tempObject.pviotEnabled);
       $scope.startDate = tempConfig.startDate
       $scope.endDate = tempConfig.endDate
       $scope.output  = tempConfig.query
       $scope.searchQueryString  = tempConfig.queryString
       $scope.type  = tempConfig.type
       if(tempConfig.actualQuery){
         self.query = tempConfig.actualQuery;
       }


       $scope.apply();

    $("#open_search").modal('hide');
...
$scope.saveSearches = function(){
    ...
    $scope.options['actualQuery'] = self.query;
    var tempData = {'colState':colState,'groupState':groupState,'sortState':sortState,'filterState':filterState,'pviotEnabled':gridOptions.columnApi.isPivotMode()};
    $scope.historicalSearchModel.tableMetaData = JSON.stringify(tempData);
    $scope.historicalSearchModel.metaData = JSON.stringify($scope.options);

    widgetService.saveSearches($scope.historicalSearchModel).then(function (response) {
        if(response.data.status){
	...
	if(self.temp != {}){
		if(self.temp.queryId == undefined){
			try{
			   ...
			   for(var i=0;i<self.query.rules.length;i++){
               				if(self.query.rules[i].value.toLowerCase() === self.temp.value.toLowerCase() && self.query.rules[i].field.toLowerCase() === self.temp.field.toLowerCase()){
               				    self.filterMessagaes.push({type:"danger",msg:"Duplicate Query Found"});
               				    $timeout(function(){
                                						self.filterMessagaes = [];
                                					},2000);
               					return false;
